...
export async function initTables() {
  ...
  try {
    ...
    await db.query(`ALTER TABLE drilling_records ADD COLUMN IF NOT EXISTS coord_system TEXT DEFAULT 'WGS-84';`);
    await db.query(`ALTER TABLE drilling_records ADD COLUMN IF NOT EXISTS coord_system_project TEXT DEFAULT 'WGS-84';`);
    await db.query(`ALTER TABLE drilling_records ADD COLUMN IF NOT EXISTS coord_system_true TEXT DEFAULT 'WGS-84';`);
    ...
    await db.query(`ALTER TABLE field_data ADD COLUMN IF NOT EXISTS coord_system TEXT DEFAULT 'WGS-84';`);

    await db.query(`ALTER TABLE primary_survey_data ADD COLUMN IF NOT EXISTS coord_system TEXT DEFAULT 'WGS-84';`);
    await db.query(`ALTER TABLE primary_survey_data ADD COLUMN IF NOT EXISTS queue INTEGER;`);
    await db.query(`ALTER TABLE primary_survey_data ADD COLUMN IF NOT EXISTS reserves_category TEXT;`);
    await db.query(`ALTER TABLE primary_survey_data ADD COLUMN IF NOT EXISTS depth REAL;`);
    await db.query(`ALTER TABLE primary_survey_data ADD COLUMN IF NOT EXISTS coordinates_msk02 TEXT;`);
    await db.query(`ALTER TABLE primary_survey_data ADD COLUMN IF NOT EXISTS coordinates_gsk2011 TEXT;`);
    ...
    if (process.env.NODE_ENV === 'production' && !process.env.ADMIN_PASSWORD) {
      console.warn('⚠️ ADMIN_PASSWORD не задан — для admin используется пароль по умолчанию. Смените его после входа!');
    }

    if (process.env.NODE_ENV !== 'production') {
      await db.query('DELETE FROM audit_log');
      await db.query('DELETE FROM user_sites');
      await db.query('DELETE FROM drilling_records');
      await db.query('DELETE FROM field_data');
      await db.query('DELETE FROM washing_data');
      await db.query('DELETE FROM assay_data');
      await db.query('DELETE FROM primary_survey_data');
      await db.query('DELETE FROM sites');
    } else {
      await db.query("DELETE FROM users WHERE role = 'driller'");
    }

    const users = [
      { id: '1', username: 'admin', password: process.env.ADMIN_PASSWORD || 'admin123', role: 'admin' },
      { id: '3', username: 'geologist', password: process.env.SEED_GEOLOGIST_PASSWORD || '123456', role: 'field_geologist' },
      { id: '4', username: 'washer', password: process.env.SEED_WASHER_PASSWORD || '123456', role: 'washer' },
      { id: '5', username: 'sampler', password: process.env.SEED_SAMPLER_PASSWORD || '123456', role: 'sampler' },
      { id: '6', username: 'chief', password: process.env.SEED_CHIEF_PASSWORD || '123456', role: 'chief_geologist' },
    ];
