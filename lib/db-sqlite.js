...
export async function initTables() {
  try {
    ...
    const migrations = [
      ...
      `ALTER TABLE drilling_records ADD COLUMN queue INTEGER`,
      `ALTER TABLE drilling_records ADD COLUMN is_drilled BOOLEAN DEFAULT 0`,
      ...
      `ALTER TABLE primary_survey_data ADD COLUMN queue INTEGER`,
      `ALTER TABLE primary_survey_data ADD COLUMN reserves_category TEXT`,
      `ALTER TABLE primary_survey_data ADD COLUMN depth REAL`,
      `ALTER TABLE primary_survey_data ADD COLUMN coordinates_msk02 TEXT`,
      `ALTER TABLE primary_survey_data ADD COLUMN coordinates_gsk2011 TEXT`,
    ];
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      sqlite.exec('DELETE FROM user_sites');
      sqlite.exec('DELETE FROM drilling_records');
      sqlite.exec('DELETE FROM field_data');
      sqlite.exec('DELETE FROM washing_data');
      sqlite.exec('DELETE FROM assay_data');
      sqlite.exec('DELETE FROM primary_survey_data');
      sqlite.exec('DELETE FROM sites');
    } else {
      sqlite.exec("DELETE FROM users WHERE role = 'driller'");
    }

    console.log('✅ Таблицы SQLite созданы + миграции выполнены');

    // Пароли по умолчанию можно переопределить через env (только для fresh-установки —
    // существующим пользователям пароль НЕ меняется, меняйте через /profile).
    // В production без ADMIN_PASSWORD будет предупреждение в лог.
